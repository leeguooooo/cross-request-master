...
function isImmersiveDoc(data) {
  ...
  if (desc.startsWith('<iframe srcdoc=')) return true;
  if (markdown.startsWith('> ⚠️ 此文档由 HTML 源生成')) return true;
  return false;
}
```
...
function tagSections() {
  const titles = document.querySelectorAll('h2.interface-title');
  titles.forEach((h2) => {
    const text = (h2.textContent || '').trim();
    const match = SECTION_LABELS.find((s) => text.includes(s.text));
    if (!match) return;
    if (h2.dataset.crmSection !== match.kind) h2.dataset.crmSection = match.kind;
    // 打标 h2 后面、直到下一个 h2.interface-title 之前的所有兄弟元素
    let sib = h2.nextElementSibling;
    while (sib && !(sib.tagName === 'H2' && sib.classList.contains('interface-title'))) {
      if (sib.dataset.crmSection !== match.kind) sib.dataset.crmSection = match.kind;
...
async function checkImmersive() {
  ...
  try {
    ...
    immersiveCache.set(apiId, 'no'); // 失败按非文档处理
  } finally {
...
| 4 | `tests/yapi-doc-immersive.test.js`（新建） | `isImmersiveDoc(data)` 纯函数单测；2-5 个用例 |
...
由于扩展运行在浏览器，主要测纯函数 `isImmersiveDoc`：
